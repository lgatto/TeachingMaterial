\section{Build-in \C interface}

\begin{frame}
  \begin{block}{The \R \C API}
    \begin{itemize}
    \item Very frequent in \R but with its quirks. 
...
\begin{frame}
  ...
  \begin{block}{\texttt{.Call}}
    \begin{itemize}
    \item Accepts any \R data structures as arguments and return values (no type checking is done though).      
...
\begin{frame}
  \begin{block}{Accessing/setting \texttt{SXP} elements}
    ...
    \item \Rfunction{VECTOR_ELT(x, i)} to access individual \texttt{SXP} elements of a \texttt{VECSXP}
    \end{itemize}
    \begin{itemize}
    \item \Rfunction{SET_STRING_ELT(out, 0, x))} to set an element in a string.
    \item \Rfunction{SET_VECTOR_ELT(out, 0, x))} to set an element in a list.
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{\url{text/gccount.c}}
  \tiny
...
<<gccountr0, eval = FALSE, tidy = FALSE>>=
gccount <- 
  function(inseq) .Call("gccount", inseq)
@
      \item Use you \C code: \Rfunction{gccount("GACAGCATCA")}
    \end{enumerate}
  \end{block}
\end{frame}
...
\begin{frame}[fragile]
  \begin{block}{Using the \Rpackage{sequences} package}
    \small
...
There is also \Rfunction{isRead(x)}, \Rfunction{isInteger(x)}, \ldots for atomics vectors.
\end{frame}
