...
and elegant syntax, code profling and testing, performance improvements via parallelisation 
...
Other considerations that need to be taken into account when several of the 
jobs to be run in parallel take different times to be completed is load balancing.
...
position 4 and \Rfunction{FUN}, i.e. \Rfunction{e} fails at the fifth iteration with the 
error

<<erroronly, eval=FALSE>>=
Error in x[-1:2] (from #3) : only 0's may be mixed with negative subscripts
@ 
