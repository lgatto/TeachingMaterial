...
Data imputation can however have citical consequences depending on the
proportion of missing values and their nature. From a statistical
point of view, missing values are classifed as *missing completely at
...
on quantitative proteomcis data).
...
If we now wanted to train a model (using the non-fromula interface):

```{r trainna}
train(X, Y)
```

(Note that the occurence of the error will depend on the model
chosen.)
...
Imputing using caret also allows to otimise the imputation based on
...
impute missing values using on other, similar non-missing rows.
...
algorithm. Scaling can also be applied direcly during model training by setting 
...
of high-varience and perpendicular predictors, preventing
collinearity.
...
```{r, eval=TRUE}
train(X, Y, preProcess = c("knnImpute", "scale", "pca"))
```
