...
Once we have trained our model, we can direcly use object of `train`
...
negatives* (FN) and *false positives8 (FP).
...
- the sensitivity (recall, TP rate): TP/(TP + FN)
- the specificity: TN/(TN + FP)
- positive predictive value (precision): TP/(TP + FP)
- negative predictive value: TN/(TN + FN)
- FP rate (fall-out): FP/(FP + TN)
...
over-fitting and hence quite popular. They howeverrequire
...
overfitting. Overfitting models noise, rather than general patterns in
the data, focusing on subtle patterns (outliers) that won't
generalise.
  
To avoid overfitting, individual decision trees are pruned. Pruning
can happen as a pre-condition when growing the tree, or afterward, by
...
- *Pre-pruning*: stop growing process, i.e stops divide-and-conquer
  ...
  predefined level), or requires a minimum number of obervations in
...
The main hyperparanter is *mtry*, i.e. the number of randomly selected
variables used at each split. 2 variables produce random models, while
100s of variables tend to be less random, but risk
overfitting. `caret` automate the tuning of the hyperparameter using a
**grid search**, which can be parametrised by setting `tuneLength`
(that sets the number of hyperparameter values to test) or direcly
...
Filter out observations with NA, but can lead to biases in data and
thus generate over-confident models. Imputation using median of
features. Works well if the data is missing at random (MAR).

```
caret::train(X, Y, preProcess = "medianImpute")
```
will do median imputation inside each fold.

If there is a bias in the missing values, i.e. data is missing not at
random (for example only small values are missing), median imputation
can lead to incorrect results.

## KNN imputation

Imputes based on other, similar non-missing rows.

```
caret::train(X, Y, preProcess = "knnImpute")
```

## Multiple preprocessing methods

It is possible to chain multiple processing methods: median, center,
scale, glmnet.

```
train(X, Y, preProcess = c("mediaImpute", "scale", "scale", "pca"))
train(X, Y, preProcess = c("mediaImpute", "scale", "scale", "spatialSign"))
```

- Start with median imputation, then try knn imputation
- Always center and scale
- Try PCA and spatial sign
- Tree-based methods don't need much pre-proc

## Low information predictors

- for example constant or random variables
- remove low variance variables

```
train(X, Y, method = "glm", preProcess = c("zv", ...)) ## remove constant (zero-variance) columns
train(X, Y, method = "glm", preProcess = c("nzv", ...)) ## nearly constant columns
```

## PCA

Used as pre-processing method, generating single set of high-varience
and perpendicular predictors, preventing collinearity.
