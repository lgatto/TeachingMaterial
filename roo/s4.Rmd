...
We can create new data types using the `setMethod` function. We need to specify both the name of the class, and which attributes the class has (in R speak: `slots`).
...
We can create a new object of this class by using `new()`:

```{r}
genseq = new("GenericSeq", name="A generic sequence", alphabet=c("A", "T"), sequence="AATTTAAATTTTT")
```

When calling `new` R will check if the supplied slots match the definition:

```{r, error=TRUE}
genseq = new("GenericSeq", name=10, alphabet=c("A", "T"), sequence="ATTTA")
```

You can access the slots directly by using a special `@` operators, or using `slots` progamatically:
...
Bioconductor packages `BiocGenerics` and `Biobase` define a large number of S4 generics used in the Bioconductor project. 
...
- [BiocGenerics generics](https://github.com/Bioconductor-mirror/BiocGenerics/tree/release-3.4/R)
- [Biobase generics](https://github.com/Bioconductor-mirror/Biobase/blob/release-3.4/R/AllGenerics.R)

### Creating S4 generics

If you think you'll need some functionality implemented differently for different classes, you might want to create a generic so that dispatching makes this invisible to the user. 

You only need to create an S4 generic if it doens't exist as an S3 generic (or a primitive as `length()`). Every S3 generic (e.g. `summary()`) has an automatic S4 generic as well. 
...
We can define our own generics:
...
NOTE: the formal arguments of the generic specify the formal arguments of all the implementations. It's good practice to always add `...` to the end to enable methods to introduce additional parameters. 

## Creating S4 methods

Once we have a generic, we can plug in our own implementation for it. 
...
1. The _user_ should *never* have to use the `@` operator. This operator is for the developer. 

2. The _user_ should *never* have to call `new()`, but create objects with functions we write as wrappers around `new()`. 
...
In our case, both methods have very simple implementations, but this gives us an opportunity to continue working on the implementation. E.g. what if we at some point decided we wanted to keep all the information in a big-data backend? We an change our internal implementation, e.g. the object might just have a single slot that is the connection to the big data backend, but we would keep these methods the same, and from user's perspective everything still works the same, but better. 

*Discussion*: do we need one method per every slot? Do we need to use methods, or can we also use functions? 

### Exercise

Implement methods for getting the setting the ```sequence`` slot of our object. 
...
It might be useful to over-ride operators such as `[`, `$`, `+`, ... All of these already have generics we just need to implement them:
...
- Same overall logic as S3, but more formal
