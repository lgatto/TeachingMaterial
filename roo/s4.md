...
We can create new data types using the `setMethod` function. We need to
...
You can access the slots directly by using a special `@` operators, or
...
Bioconductor packages `BiocGenerics` and `Biobase` define a large number
of S4 generics used in the Bioconductor project.
...
-   [BiocGenerics
    generics](https://github.com/Bioconductor-mirror/BiocGenerics/tree/release-3.4/R)
-   [Biobase
    generics](https://github.com/Bioconductor-mirror/Biobase/blob/release-3.4/R/AllGenerics.R)

### Creating S4 generics

If you think you'll need some functionality implemented differently for
different classes, you might want to create a generic so that
dispatching makes this invisible to the user.

You only need to create an S4 generic if it doens't exist as an S3
generic (or a primitive as `length()`). Every S3 generic (e.g.
...
NOTE: the formal arguments of the generic specify the formal arguments
of all the implementations. It's good practice to always add `...` to
the end to enable methods to introduce additional parameters.
...
2.  The *user* should *never* have to call `new()`, but create objects
    with functions we write as wrappers around `new()`.
...
In our case, both methods have very simple implementations, but this
gives us an opportunity to continue working on the implementation. E.g.
what if we at some point decided we wanted to keep all the information
in a big-data backend? We an change our internal implementation, e.g.
the object might just have a single slot that is the connection to the
big data backend, but we would keep these methods the same, and from
user's perspective everything still works the same, but better.
...
Implement methods for getting the setting the \``sequence` slot of our
object.
...
can use to find out information about your and other people's classes.
    ...
    ##     letters = strsplit(x@sequence, "")[[1]]
    ##     reversed = paste(rev(letters), collapse = "")
    ##     new("GenericSeq", name = paste(x@name, "--reversed"), alphabet = x@alphabet, 
    ##         sequence = reversed)
    ## }
    ## 
    ## Signatures:
...
It might be useful to over-ride operators such as `[`, `$`, `+`, ... All
of these already have generics we just need to implement them:
    ...
    ## <bytecode: 0x7fc14d859a08>
    ## <environment: 0x7fc14d851878>
