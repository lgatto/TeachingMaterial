...
of S4 generics used in the Bioconductor project, and are the first place
to look for generics to re-use.
...
in a big-data backend? We can change our internal implementation, e.g.
the object now has a single slot that is the connection to the big data
backend, but we would keep these methods the same, and from user's
perspective everything still works the same, but better.
...
Implement methods for getting the setting the \``sequence` property of
our `GenericSeq` object.
...
of these already have generics we just need to implement them:
    ...
    ## <bytecode: 0x7fc80cadd208>
    ## <environment: 0x7fc80cad5078>
...
the `roxygen2` package. It has custom tags to document classes and
methods:
...
documentation for the generic and the method into a same `.rd` file name
`sequence-methods`. You can now generate the the `NAMESPACE` and `.rd`
documentation files by running `roxygenise("path/to/your/package")`.
