...
\lstset{
  ...
  emph={[2]as}, emphstyle={[2]\color{green}},
  emph={[3]as,wrap}, emphstyle={[3]\color{blue}},
...
\begin{frame}
  \begin{block}{sugar}
    ...
    \item Functions: \texttt{seq_len}, \texttt{all}, \texttt{any}, \texttt{sapply}, \texttt{rnorm}, \texttt{abs}, ...
    \end{itemize}    
  \end{block}
\end{frame}
...
\lstset{
  language=C++,
  emph={NumericVector,CharacterVector}, emphstyle=\color{red},
...
\begin{frame}[fragile]
  \begin{exercise}{Translate these \R functions into \C or \Cpp}
    \tiny
...
## examples of sapply, calling functions
@ 
  \end{exercise}
\end{frame}
