...
\begin{frame}
  \begin{block}{Demonstration}
    \begin{itemize}
      \item Let's inspect the \texttt{sequences/tests/sequences-test.R} and \texttt{sequences-test.Rout.save} files. 
        When actually \Rfunction{check}ing the package, the resulting \texttt{sequences-test.Rout} file is in 
        \texttt{sequences.Rcheck/tests/}.
      \item Let's play with \Rpackage{testthat} interactively.
    \end{itemize}
  \end{block}
\end{frame}
