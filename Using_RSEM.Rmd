...
This short presentation will take you through a short example of RNA quantification using RSEM. You will need to have the following software installed on your machine (Bryan should have those setup on the server):
...
for(i in 1:nrow(pd_small)) {
  ...
  SRX_number[i] <- gsub(".*=SRX", "SRX", gd@header$relation[1])
	...
	system(paste0('ascp -i ~/Applications/Aspera\\ Connect.app/Contents/Resources/asperaweb_id_dsa.openssh -k 1 -T -l200m ', aspera_url, " RSEM_test/SRA"))
}
...
cd RSEM_test/RSEM
parallel -j 2 rsem-calculate-expression --bowtie2 -p 2 {} ../Reference_Genome/hg19 {/.} ::: ../FASTQ/*.fastq
```
...
RSEM results are assembled: `r !cond_eval`
