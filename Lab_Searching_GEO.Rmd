...
We will need to query the GEOmetadb database. Today we will explore this database and practice various ways to query it.
...
However, since we also grouped '`by=manufacturer` in our `data.table`, we could have simply use the `SQL` query:
...
Query the manufacturers with a simpler `SQL` command ... grouping with `by` and ordering with `setkey` in `data.table`...
