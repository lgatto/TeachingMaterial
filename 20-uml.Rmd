...
the data, or apply princpal component analysis (next section). Scaling
...
- preprocessing for other ML algorithms, and
...
ideal candiates used to visualise the data along these reducd number
of informative dimensions.

## How does it work

Principal Component Analysis (PCA) is a technique that transforms the
original n-dimentional data into a new n-dimentional space. 

- These new dimensions are linear combinations of the orignal data,
...
```{r histpc1, echo=FALSE, fig.caption=""}
...
A **biplot** features all original points remaped (rotated) along the
first two PCs as well as the origial features as vectors along the
...
> - It is often useful to produce constom figures using the data
...
We haven't looked at other `prcmop` parameters, other that the first
...
<details>
```{r scalepcasol, fig.with=12}
par(mfrow = c(1, 2))
biplot(prcomp(mtcars, scale = FALSE), main = "No scaling")  ## 1
biplot(prcomp(mtcars, scale = TRUE), main = "With scaling") ## 2
```

Without scaling disp and hp are the features with the highest loadings
along PC1 and 2 (all others are negligible), which are also those with
the highest units of measurement. Scaling removes this effect.
</details>

## Final comments

- Missing values: drop or impute
- Categorical data: drop or encode as dummy numerical values


# t-Distributed Stochastic Neighbour Embedding

[t-Distributed Stochastic Neighbour Embedding](https://lvdmaaten.github.io/tsne/) (t-SNE)
is a *non-linear* dimensionality reduction techique, i.e. that
