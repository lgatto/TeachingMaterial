...
Below, we firs try to replace the expression matrix with an invalid replacement value 
...
it the S4 system provides improved stability and usability in the future. 
The design and usage of an efficient class system however implies to think about 
the needs of the user role before code, as it involves some commitment in the design decisions. 
