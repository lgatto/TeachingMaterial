...
These must match exactly, as we write a method for that sepcific generic. 
...
This is the same name that we will have to use when writing the \Robject{defintion} 
of our method.
...
using an environement instead of a matrix, the \Robject{ma@marray} is going 
