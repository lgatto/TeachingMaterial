...
and makes it possible to easily breaks the validity of our object.
...
Replacement method are always names by concatanating the name of the method and the arrow 
assigment operator. If we wish to write a replacement method to replace the slot that can 
...
not break the validity of our object, as we explicitely test the validity of our updated 
...
To find out more about a class you are using withtout reading its source code, 
...
To obtian all the methods that are available for a given function name of for 
