...
} introduces \R object-oriented programming using microarrays as a use case. 
...
data that is to be used for subsequent computational analysis, think of what is going 
...
We will also set the same names for the intensity matrix columns and the sample meta-data rows 
as well as the matrix rows and feature meta-data row. 
...
It now becomes clear that we have two different kind of roles. 
The \textit{developer} is the one who creates the class and knows the implementation and
...
replacement or update of the content of a slots. It is of course possible to 
perform such an operation by accessing the slot content directly, as illustrated below, 
although this is not advised as it breaks the encapsulation of our data 
and makes it possible to easily break the validity of our object.
...
There is a special type of method, called replacement methods, that can be implemented to 
obtain the desired behaviour in a clean and controlled way. The replacement method 
provides the convenient \Rfunction{slot(object)<-} syntax. 

Replacement method are always names by concatenating the name of the method and the arrow 
assignment operator. If we wish to write a replacement method to replace the slot that can 
...
Another important specificity of replacement method is that they always have (at least) two arguments; 
the object to be updated, that we will name \Robject{object} and the replacing data, 
...
To use our replacement function, we need to make sure that the new \Robject{value} will 
not break the validity of our object, as we explicitly test the validity of our updated 
\Robject{object} inside the replacement method.
...
To find out more about a class you are using with tout reading its source code, 
...
it guarantees improved stability and usability in the future.
The design and usage of an efficient class system however implies to think about 
the needs of the user role before code.
