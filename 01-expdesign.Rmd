...
## **Hypothesis-driven** vs hypthesis-free (-generating) experiments
...
- Do cells treated with drug A for 20 or 40 min express protein X but
  not Y? 
- Let's test all these drugs over as many timepoints as we can and see
...
Study: *in the wild*, much more variablility in the data,
...
- Noise vs random variation vs systematic biais.

- Technical vs biological variability: the variability we measure is
  composed of technical and biological variability. If the former
  dominates, we won't learn anything about biology.
...
We can only assess any effect if we consider the variability in our
experiment.
...
**Cofounding batch effect**: any observed differences between the
...
- Two-way design: treatent (A, B, control) x genetic background (WT, MT)
...
deltas <- c(2, 2, 2, 1)
sds <- c(1, 1, 2, 1.5)
sig <- c(0.01, 0.05, 0.05, 0.05)

res <- matrix(NA, 8, 5)
rownames(res) <- seq(0.6, 0.95, 0.05)
for (i in 1:4) {
    res[, i] <- sapply(seq(0.6, 0.95, 0.05),
...
axis(side = 1, at = 1:8, labels = rownames(res))
legend("topleft",
...
- Can you identify cofounding/batch effects?
